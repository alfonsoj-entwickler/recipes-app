...
export default function GenerateAI() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (prompt.trim() === "") {
      showNotification({
        text: "Texfield needs a question",
        error: true,
      });
      return;
    }
  ...
  return (
    <>
      <h1 className="text-6xl font-extrabold">Chatgpt Recipes</h1>
      ...
      <div className="max-w-4xl mx-auto">
        <form onSubmit={handleSubmit} className="flex flex-col space-y-3 py-10">
          <div className="relative">
            <input
              ...
              placeholder="Generate a recipe with ingredients. Ex. Drink with Tequila and Orange"
            />
            <button
              type="submit"
              aria-label="Enviar"
              ...
              className={`cursor-pointer absolute top-1/2 right-5 transform -translate-x-1/2 -translate-y-1/2 ${
                isGenerating ? "cursor-not-allowed opacity-50" : "opacity-100"
              }`}
            >
              <svg
        ...
        {isGenerating && (
          <p className="text-center animate-blink">Genereting ...</p>
        )}
